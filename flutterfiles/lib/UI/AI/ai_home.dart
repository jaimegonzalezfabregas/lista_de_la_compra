...
class AiMetadata {
  ...
  late final String sizeGb;
  late final Uri infoLink;
  ...
  AiMetadata(Map<String, dynamic> seed) {
    ...
    notes = seed["notes"];
    id = seed["id"];
    downloadUrl = Uri.parse(seed["download_url"]);
    sizeGb = seed["size_gb"];
    infoLink = Uri.parse(seed["info_link"]);
  ...
  void download() async {
    try {
      if (downloaded || _response != null) {
        throw "This model is already donwloaded";
      }

      stateStream.add("Starting download");

      _response = await http.Client().send(http.Request('GET', downloadUrl));
